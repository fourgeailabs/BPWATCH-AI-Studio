...
object RecordScheduler {
    ...
    /** Fixed 10-minute cadence — relaxed to 30 min in battery-saving mode (< 20%). */
    private const val INTERVAL_MS = 10 * 60_000L

    fun schedule(context: Context) {
        val am = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        am.cancel(operation(context))
        val interval = WatchBatterySaver.getRecordIntervalMs(context)
        scheduleNextAt(context, System.currentTimeMillis() + interval)
        Log.i(TAG, "Recording scheduled every ${interval / 60_000} min (saverActive: ${WatchBatterySaver.isBatterySaverActive(context)})")
    }
    ...
    /** (Re)applies recording schedule respecting battery-saving adjustments. */
