    ...
    private fun beginMonitoring() {
        ...
        mon.onSample = { hr ->
            if (hr in 25f..250f) {
                ...
                // Throttled live tick so the phone can mirror the watch (throttled further when battery < 20%).
                val nowLive = System.currentTimeMillis()
                val liveTickMs = WatchBatterySaver.getLiveTickIntervalMs(applicationContext)
                if (nowLive - lastLiveSendMs >= liveTickMs) {
        ...
        val uploadRunnable = object : Runnable {
            override fun run() {
                uploadWindowAverage()
                val nextInterval = WatchBatterySaver.getUploadIntervalMs(applicationContext)
                handler.postDelayed(this, nextInterval)
            }
        }
        handler.postDelayed(uploadRunnable, WatchBatterySaver.getUploadIntervalMs(applicationContext))
    }
