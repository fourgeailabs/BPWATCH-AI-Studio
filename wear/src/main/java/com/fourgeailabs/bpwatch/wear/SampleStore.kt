...
object SampleStore {
    ...
    fun append(context: Context, sample: Sample) {
        try {
            ...
            java.io.FileOutputStream(file, true).use { outs ->
    ...
    fun deleteUpTo(context: Context, timestamp: Long) {
        deleteRange(context, Long.MIN_VALUE, timestamp)
    }

    /**
     * Deletes every sample with timestamp within [minTimestamp]..[maxTimestamp]
     * by rewriting the file keeping only samples outside that window.
     */
    @Synchronized
    fun deleteRange(context: Context, minTimestamp: Long, maxTimestamp: Long) {
        try {
            val keep = readAll(context).filter { it.timestamp !in minTimestamp..maxTimestamp }
            val file = file(context)
            if (keep.isEmpty()) {
                file.delete()
                return
            ...
            tmp.renameTo(file)
            Log.i(TAG, "Pruned to ${keep.size} samples after deleteRange($minTimestamp, $maxTimestamp)")
        } catch (e: Exception) {
            Log.w(TAG, "deleteRange failed", e)
        }
    }
