...
class PhoneListenerService : WearableListenerService() {
    ...
    private fun handleWristSet(event: MessageEvent) {
        scope.launch {
            try {
                val map = DataMap.fromByteArray(event.data)
                val wrist = map.getString(Link.KEY_WRIST).orEmpty()
                if (wrist.isNotEmpty()) {
                    MonitoringPrefs(applicationContext).setWrist(wrist)
                }
            } catch (_: Exception) {
            }
        }
    ...
    private fun handleBatteryState(event: MessageEvent) {
        scope.launch {
            try {
                val map = DataMap.fromByteArray(event.data)
                val level = map.getInt(Link.KEY_BATTERY_LEVEL)
                val saver = map.getBoolean(Link.KEY_BATTERY_SAVER_ACTIVE)
                WatchLiveState.updateBatteryState(level, saver)
            } catch (_: Exception) {
            }
        }
    }
    ...
    private fun handleHrReading(event: MessageEvent) {
        scope.launch {
            try {
                ...
                if (map.containsKey(Link.KEY_BATTERY_LEVEL)) {
                    val batLevel = map.getInt(Link.KEY_BATTERY_LEVEL)
                    val saver = if (map.containsKey(Link.KEY_BATTERY_SAVER_ACTIVE)) {
                        map.getBoolean(Link.KEY_BATTERY_SAVER_ACTIVE)
                    } else {
                        batLevel < 20
                    }
                    WatchLiveState.updateBatteryState(batLevel, saver)
                }
                ...
                val posture = if (map.containsKey(Link.KEY_POSTURE)) {
                    ...
                    Link.Posture.SITTING_DOWN
                }
                val activity = if (map.containsKey(Link.KEY_ACTIVITY)) {
                    map.getString(Link.KEY_ACTIVITY) ?: Link.ActivityState.SITTING
                } else {
                    Link.ActivityState.SITTING
                }

                val monitoringPrefs = MonitoringPrefs(applicationContext)
                val measurementLocation = if (map.containsKey(Link.KEY_WRIST_LOCATION)) {
                    map.getInt(Link.KEY_WRIST_LOCATION)
                } else {
                    monitoringPrefs.getMeasurementLocation()
                }

                if (map.containsKey(Link.KEY_WRIST)) {
                    val wrist = map.getString(Link.KEY_WRIST)
                    if (!wrist.isNullOrEmpty()) {
                        monitoringPrefs.setWrist(wrist)
                    }
                }
                ...
                if (model != null) {
                    ...
                    // Publish to Health Connect (with detected posture and body side location).
                    try {
                        val hc = HealthConnectManager(applicationContext)
                        if (hc.isAvailable && hc.hasPermissions()) {
                            val hcBodyPosition = when (posture) {
                                Link.Posture.STANDING_UP -> androidx.health.connect.client.records.BloodPressureRecord.BODY_POSITION_STANDING_UP
                                Link.Posture.LYING_DOWN -> androidx.health.connect.client.records.BloodPressureRecord.BODY_POSITION_LYING_DOWN
                                Link.Posture.RECLINING -> androidx.health.connect.client.records.BloodPressureRecord.BODY_POSITION_RECLINING
                                Link.Posture.SITTING_DOWN -> androidx.health.connect.client.records.BloodPressureRecord.BODY_POSITION_SITTING_DOWN
                                else -> androidx.health.connect.client.records.BloodPressureRecord.BODY_POSITION_SITTING_DOWN
                            }
                            val hcLocation = when (measurementLocation) {
                                Link.MeasurementLocation.RIGHT_WRIST -> androidx.health.connect.client.records.BloodPressureRecord.MEASUREMENT_LOCATION_RIGHT_WRIST
                                Link.MeasurementLocation.LEFT_WRIST -> androidx.health.connect.client.records.BloodPressureRecord.MEASUREMENT_LOCATION_LEFT_WRIST
                                Link.MeasurementLocation.RIGHT_UPPER_ARM -> androidx.health.connect.client.records.BloodPressureRecord.MEASUREMENT_LOCATION_RIGHT_UPPER_ARM
                                Link.MeasurementLocation.LEFT_UPPER_ARM -> androidx.health.connect.client.records.BloodPressureRecord.MEASUREMENT_LOCATION_LEFT_UPPER_ARM
                                else -> androidx.health.connect.client.records.BloodPressureRecord.MEASUREMENT_LOCATION_LEFT_WRIST
                            }
                            hc.writeBloodPressure(
                                sys = sys,
                                dia = dia,
                                time = Instant.ofEpochMilli(timestamp),
                                bodyPosition = hcBodyPosition,
                                measurementLocation = hcLocation,
                            )
                            val postureName = when (posture) {
                                Link.Posture.STANDING_UP -> "Standing"
                                Link.Posture.LYING_DOWN -> "Lying down"
                                Link.Posture.RECLINING -> "Reclining"
                                Link.Posture.SITTING_DOWN -> "Sitting"
                                else -> "Sitting"
                            }
                            com.fourgeailabs.bpwatch.mobile.healthconnect.SamsungHealthSyncState.onWearablePacketReceived(
                                hr = hr,
                                sys = sys,
                                dia = dia,
                                posture = postureName,
                                wrist = if (measurementLocation == Link.MeasurementLocation.RIGHT_WRIST) "Right" else "Left",
                            )
                        }
                    } catch (_: Exception) {
                    }
